...
for i in range(len(NN)):  # 遍历
    # z值线，即实际数据。
    axs2.plot(Y[i],X[i],np.array(TAUI)[i],color='b',alpha=0.3)
    # 0值线（z=0），与“地面”连接。
    axs2.plot(Y[i], X[i], np.zeros_like(np.array(TAUI)[i]), color='gray', alpha=0.5) 
    # 绘制有颜色的平面：本质是填充z值与0值之间的区域。
    polygon = [
        [Y[i, 0], X[i, 0], 0],    # 左下
        [Y[i, -1], X[i, -1], 0],  # 右下
    ...
    for j in range(len(alpha2)-1, -1, -1):  # 依次添加点，使得polygon成为一个完整的闭合多边形
        polygon.append([Y[i, j], X[i, j], np.array(TAUI)[i, j]])
    axs2.add_collection3d(Poly3DCollection([polygon], color='b', alpha=0.3))
    for k in [0,round((prec-1)*2/5),prec-1]:
        axs2.scatter(Y[i, k], X[i, k], np.array(TAUI)[i, k],color='b')
    # 标注数字（z值）
    #for k in [0,round((prec-1)/3),round((prec-1)*2/3),prec-1]:
        #axs2.text(Y[i, k]+1.2, X[i, k], np.array(TAUI)[i, k]-1, f'{np.array(TAUI)[i, k]:.2f}',
                 #color='b', ha='center', size=7) 
# 用虚线将需要标注的时间（y）连起来
for k in [0,round((prec-1)*2/5),prec-1]:
    axs2.plot(Y[:, k], X[:, k], np.array(TAUI)[:, k], linestyle='--', color='b') 

for i in range(len(NN)):  # 遍历
    # z值线，即实际数据。
    axs2.plot(Y[i],X[i],np.array(TAUC)[i],color='r',alpha=0.3)
    # 0值线（z=0），与“地面”连接。
    axs2.plot(Y[i], X[i], np.zeros_like(np.array(TAUC)[i]), color='gray', alpha=0.5) 
    # 绘制有颜色的平面：本质是填充z值与0值之间的区域。
    polygon = [
        [Y[i, 0], X[i, 0], 0],    # 左下
        [Y[i, -1], X[i, -1], 0],  # 右下
    ...
    for j in range(len(alpha2)-1, -1, -1):  # 依次添加点，使得polygon成为一个完整的闭合多边形
        polygon.append([Y[i, j], X[i, j], np.array(TAUC)[i, j]])
    axs2.add_collection3d(Poly3DCollection([polygon], color='r', alpha=0.3))
    for k in [0,round((prec-1)*2/5),prec-1]:
        axs2.scatter(Y[i, k], X[i, k], np.array(TAUC)[i, k],color='r')
    # 标注数字（z值）
    #for k in [0,round((prec-1)/3),round((prec-1)*2/3),prec-1]:
        #axs2.text(Y[i, k]-1.2, X[i, k], np.array(TAUC)[i, k]+1, f'{np.array(TAUC)[i, k]:.2f}',
                 #color='r', ha='center', size=7)         
# 用虚线将需要标注的时间（y）连起来
for k in [0,round((prec-1)*2/5),prec-1]:
    axs2.plot(Y[:, k], X[:, k], np.array(TAUC)[:, k], linestyle='--', color='r') 

axs2.set_xticks([5,10,15,20,25])
axs2.set_xlabel('the number of data factor suppliers')
axs2.set_ylabel('the value of the parameter alpha2')
axs2.set_zlabel('exchange pricing')
